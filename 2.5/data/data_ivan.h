...
//might need to move this prototype to a separate file
typedef double* C;
...
//this is the maxmin option hardcoded, constant doesn't work
const int     OPTIM             = 4;
...
const int     TYPE_SCHEME       = 2;        //- space scheme : 1 or LF ; 2 or ENO2
// const int     TYPE_RK           = RK2;         //- time scheme (ENO2 case) : 1 or ENO2RK1, 2 or ENO2RK2,  3 or ENO2RK3
const int     TYPE_RK           = 2;         //- time scheme (ENO2 case) : 1 or ENO2RK1, 2 or ENO2RK2,  3 or ENO2RK3
...
//----------------------------
const double  DT                = 5e-3;         //- if DT=0 the program will compute a DT based on space steps.
      double  CFL               = 0.5;

const int     NN=750;
      int     ND[DIM]           = {NN, NN};
      double  XMIN[DIM]         = {0.0, 0.0};
      double  XMAX[DIM]         = {1.5, 1.5};           //- bound of the domain
...
//-----------------------------
//- BOUNDARY : 0 (Void, for FD only) or 1 (Dirichlet, using g_border, for SL/FD) or 2 (Vxx=0, for SL/FD)
//-----------------------------
...
const double  COUPE_VALS[DIM] = {0.,1.}; //- if COUPE_DIM[d]=0 then make precise the value of the cut in direction x_d.
...
inline void dynamics2(const double* x, C u, C u2, double t, double* res) {
	...
	double  compet_term = 1.0 / (1.0 + new_beta * (n_1 * new_M_1 + n_2 * new_M_2));
	
	res[0] = new_M_1 * ((1.0 - u_1) * compet_term * new_alpha / (new_M_1 + new_k) - gamma_rate);
	res[1] = new_M_2 * ((1.0 - u_2) * compet_term * new_alpha / (new_M_2 + new_k) - gamma_rate);
...
inline double distributed_cost2(const double* x, C u, C u2, double t) {
	...
	return  compet_term * new_alpha * exp(-mu_rate * (T - t)) *
			(u_2 * new_M_2  / (new_M_2 + new_k) - u_1 * new_M_1 / (new_M_1 + new_k));
