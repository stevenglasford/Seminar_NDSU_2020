...
void HJB::saveCoupeMex (const char* file, const int* coupe, const double* val)
{
  if(my_rank==0)
  ...
  if(PRINTBUG && my_rank==0)
    printf("saveCoupeMex  : Saving (%19s) on axes [",file);
  testval = new int[dim];
  for(d=0;d<dim;++d){
    if(coupe[d]){
      testval[d] = -1;
  ...
  //- xx contains an exact x, where we interpolate v[x].
  double xx[dim];
  ...
  pFile = fopen (file,"w");
  if(pFile==NULL)
    cout << "10.Impossible to write in the given file!" << endl;
  else{
    for(i=0; i<mesh->inn_nbPoints; i++){
      ...
      if(test){
        ...
        //value=(*Vex)(t, (mesh->*(mesh->getcoords)) (rk));
        value=(*Vex)(t, xx);
