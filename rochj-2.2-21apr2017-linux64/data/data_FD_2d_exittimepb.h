...
const int     TOPT_TYPE         = 1;       // 0= min time , 1= exit time
...
// stopping criteria
double        min_TRAJ_STOP     = -10.00;   //- to stop traj reconstruction when val(x) <=min (val=topt here)
double        max_TRAJ_STOP     =  10.00;   //- to stop traj reconstruction when val(x) >=max (val=topt here)
