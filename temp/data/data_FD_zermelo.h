...
const char    NAME[200]         = "data_FD_zermelo.h;  Date: Apr. 2014\nO. Bokanowski, A. Desilles, H. Zidani";
...
const int     SAVE_VF_FINAL     = 0;         //- 1 to save last VF file as "VF.dat" 
...
//int TRAJPT = 1;     double initialpoint[2]={0.0, 1.5};     //- Initial point for HJB [difficult case]
//int TRAJPT = 2;     double initialpoint[2*DIM]={-4.0, -1.0, -3.0, 0.5};     //- Initial point for HJB (simple)
//int TRAJPT = 2;     double initialpoint[2*DIM]={-3.0, 1.8,  0.0, 1.5};     //- Initial point for HJB (more difficult)
int TRAJPT = 3;     double initialpoint[3*DIM]={-3.0, 1.8, 0.0, 1.5, -3.0, -1.50};     //- Initial point for HJB <--
//int TRAJPT = 1;     double initialpoint[1*DIM]={ 0.0, 1.50};     //- Initial point for HJB
//int TRAJPT = 1;     double initialpoint[2*DIM]={-0.5, 1.25};     //- Initial point for HJB
//int TRAJPT = 1;     double initialpoint[2*DIM]={ 0.0, 1.50};     //- Initial point for HJB
...
int TRAJ_METHOD=0;
double  time_TRAJ_START=0; 
//int TRAJ_METHOD=1; 
//double  time_TRAJ_START= -INF;

// stopping criteria
double        min_TRAJ_STOP      = 0.0;    //- to stop traj reconstruction when val(x)<=min (val=topt here)
double        max_TRAJ_STOP      = 1e4;    //- to stop traj reconstruction when val(x)>=max (val=topt here)
int           TARGET_STOP        = 1;      //- to stop traj reconstruction if g_target(x)<=0
inline double g_target(double t, const double* x){ return v0(x)-(0.0);}  
