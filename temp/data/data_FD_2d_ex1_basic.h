...
//-    u(0,x)=u0(x)
//-    Basic approach : using control discretisation (COMMANDS=1)
//----------------------------------------
#include "data_default.h"	  // for basic examples
//----------------------------------------

const char    NAME[150]         = "data_FD_2d_ex1_basic.h (Eikonal equation example) Feb. 2013\nAuthors   : O. Bokanowski, H. Zidani";
...
//const int     TYPE_SCHEME       = NBEE_OCOPT2;	//- this choice allows for large NC number (control discretisation) without more cost.
...
const double                   T= 1.00;        //- Terminal time
...
const int     cDIM              = 1;
const int     NCD[cDIM]         = { 20 }; 
const double  UMIN[cDIM]        = { 0.};
const double  UMAX[cDIM]        = { 1.00*2.*pi};

/*
//- test:
const int     cDIM              = 2;
const int     NCD[cDIM]         = { 50 , 2};
...
inline void feedback(double t, const double* x, const double* p, C& u){  //- only needed for NBEE-OC
  ...
  //- Example 
  //u[0]=1.0;
  //- Example with control u = argmax (H(x,p) = max( - p . f )) 
  //- =>  u = argmax(-p*(cos(u),sin(u)))  dans [0, 2*pi]
  //- =>  (cos(u),sin(u))=-p/|.| 
  //double ff[DIM];
  u[0]=atan2(-p[1],-p[0]);
  //printf("-p[0]=%8.5f, -p[1]=%8.5f; ",-p[0],-p[1]);
  //printf("u[0]=%8.5f\n",u[0]);
  //exit(1);
  //if (-p[1]<=0 && -p[0]<=0){
  if (u[0]<=0){
    u[0]+=2*pi; //- result of atan2 is <=0 in [-pi,pi] , so here we recast into [,2pi]
  }
  //u[1]=1.0;
}
