...
private:
  ...
  int       TYPE_SCHEME;  //- MODIF-M
  int       DEBUG_HJB_FD; //- MODIF-M

  double    *vinter;  /*! for min/max (control) appproach [SIZE] */
...
private:
  ...
  //- NBEE stable version [src/HJB_FD_NBEE.h; src/HJB_FD_NBEE.cpp]
  void      HJB_NBEE                (double t, double dt, double *vin, double *vout);
  void      L_split_NBEE            (double t, double deltat, int dimen, double* vin, double* res, int nc1, int nc2);

  //- NBEE stable version - one control [src/HJB_FD_NBEE_oc.h] 
  void      HJB_NBEE_oc             (double t, double dt, double *vin, double *vout);
  void      L_split_NBEE_oc         (double t, double deltat, int dimen, double* vin, double* res, int nc1, int nc2);

  //- NBEE stable version - precomputed one control [src/HJB_FD_NBEE_ocopt.h] 
  void      HJB_NBEE_ocopt          (double t, double dt, double *vin, double *vout);
  void      L_split_NBEE_ocopt      (double t, double deltat, int dimen, double* vin, double* res, int nc1, int nc2);
  ...
  /*!
   * \brief ENO3 RK scheme
   */
  void      ENO3_RK1_omp            (double t, double dt, double *vin, double *vout);
  void      ENO3_RK2_omp            (double t, double dt, double *vin, double *vout);
  void      ENO3_RK3_omp            (double t, double dt, double *vin, double *vout);
