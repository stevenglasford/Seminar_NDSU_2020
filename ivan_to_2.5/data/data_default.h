...
//- note : {coupe,coupeex,coupetopt}.dat savings use the same step as coupe savings: SAVE_COUPE_ALL_STEP
      ...
      int     SAVE_TOPT_ALL      = 0;
      int     SAVE_TOPT_FINAL    = 1;
      int     SAVE_COUPE_ALL_STEP= 50; //- can be different from SAVE_VF_ALL_STEP; used for all "coupe" files
      int     SAVE_COUPE_ALL     = 1;
      int     SAVE_COUPE_FINAL   = 1;
      int     SAVE_COUPEEX_ALL   = 0;
      int     SAVE_COUPEEX_FINAL = 1;
      int     SAVE_COUPETOPT_ALL = 0;
      int     SAVE_COUPETOPT_FINAL=1;
      int     SAVE_VF_ONSET_ALL    = 0; //- savings iteration step for {VF,Vex,topt (& onset-files) is defined by "SAVE_VF_ALL_STEP" 
      int     SAVE_VF_ONSET_FINAL  = 0; //- to save final value on some user defined points (such as Xuser.txt)
      int     SAVE_VEX_ONSET_ALL   = 0;
      int     SAVE_VEX_ONSET_FINAL = 0;
      int     SAVE_TOPT_ONSET_ALL  = 0;
      int     SAVE_TOPT_ONSET_FINAL= 0;
      int     NBPOINTS_Xuser    = 0;           //- number of user-defined points where to also compute value.
...
const int     SAVE_COUPE_INTERPOLATION=1; //- if 1 then uses interpolation. Use 0 for testing purposes (to compare VFxx.dat and coupexx.dat files)
...
// -------------------
      int     FORMAT_FULLDATA         = 1; //- add (at left) the list of indexes corresponding to the current point {files VF.dat, etc.}
      int     SHOW_COORDINATES        = 0; //- add (at right) the list of coordinates on each line of the {VF.dat, etc.} files 
...
// -------------------
double        C_THRESHOLD       = 0.1;//10.0;    //- Error threshold for error computations
...
// --------------------------------
      int     EXTERNALV0        = 0;           //- if 1 then starts computation from data in VF.dat and not from the v0 function
