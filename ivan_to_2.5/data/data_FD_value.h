//-------------------------------------------
//- Exemple of "value" computation - SEPT 2011 / JUN 2012 / NOV 2015 / FEB 2018
//-------------------------------------------
...
//-  phi = ball
//-  K = (square obstacle)
//-  Controls :  a=(u,v) in [0, 2pi] * [0,Vmax]
//-  
//----------------------------------------
...
//- initialisation 
//----------------------------
const int     EXTERNALV0        = 0;           //- 1: for initialisation from data file VF.dat;  0: from function v0()
...
//const int     VALUE_PB          = 1; //- 1 to compute value z such that  0 = w(t,x,z)=v(t,x)-z (assumes DIM=d+1, x in R^d, z in R^1)
//const int     SAVE_VALUE_ALL    = 1; //- 1 to save value v(t,x) at  all  time iterations (assumes VALUE_PB=1)
//const int     SAVE_VALUE_ALL_STEP=1000;
...
// ---------------------------
const int     VALUE_PB           = 1; //- to compute value v(t,x) s.t. 0 = w(t,x,z)=v(t,x)-z (assumes DIM=d+1, x in R^d, z in R^1)
const int     SAVE_VALUE_ALL     = 0; //- to save at each iteration the value v(t,x) (assumes VALUE_PB=1)
//const int     SAVE_VALUE_ALL_STEP= 100; //- [OBSOLETE] - step to save final value v(t,x) (case VALUE_PB=1) 
const int     SAVE_VALUE_FINAL   = 1; //- to save final value v(t,x) (case VALUE_PB=1)
...
const int     SAVE_MPI_FULL      = 0; //- save VFxx.dat (and toptyy.dat) -full matrices files- in the MPI mode, as in SEQ mode (names: VFxx_PROCyy.dat files)
...
// -------------------
const int     FORMAT_FULLDATA         = 0; //- for savings: add (at left) the list of indexes corresponding to the current point {files VF.dat, etc.}
...
double        time_TRAJ_START   = 0.0;
// stopping criteria
double        min_TRAJ_STOP     =-1000.00; //- to stop traj reconstruction when val(x) <=min
...
//const int TRAJPT  = 2;  const double initialpoint[TRAJPT*DIM] = {-2.0,-1.5,Zmax, -1.0,1.0,Zmax };
...
//--------------------------------
//- border: (FD) number of ghost cells in each direction (default is {2,2,...} for FD)
//--------------------------------
